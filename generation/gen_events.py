...
class EventsGeneration:
    ...
    __oneEvent = dict.fromkeys(["date", "type", "name", "members", "place"])  # Генерация ключей одного события
    ...
    def __date_generation(self, date_key):
        ...
        date_key["date"] = str(self.start_date+datetime.timedelta(seconds=random.randrange(seconds_range)))
        return date_key
    ...
    def __name_generation(self, name_key):
        rand_name = ''.join(random.choice(string.ascii_lowercase + string.digits + ' ') for j in range(random.randint(0, 150)))
