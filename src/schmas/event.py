...
from src.error.input_errors import (
    DateErrors,
    TypesError,
    NamesError,
    MembersError,
...
class Event(BaseModel):
    ...
    def validate_date(cls, date):
        correct_date = (
            str(datetime.datetime.strptime(date, "%Y-%m-%d %H:%M:%S")) == date
        )
        if correct_date:
            return date
        else:
    ...
    def validate_name(cls, name):
        special_characters = (
            len([index for index in string.punctuation if index in name]) == 0
        )
    ...
    def validate_members(cls, members):
        elements_not_numbers = len(members) == len(
            [i for i in members if type(i) != int]
        )
