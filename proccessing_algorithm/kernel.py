...
class EventsHandler:
    ...
    def working_with_events(self, raw_list):
        ...
        for item in range(len(raw_list)):
            if self.__validate_keys(raw_list[item]) == raw_list[item]:
                ...
                self.__result_json[self.__date_keys(raw_list[item])].append(
                    raw_list[item]
                )
                self.__result_json[self.__date_keys(raw_list[item])] = sorted(
                    self.__result_json[self.__date_keys(raw_list[item])],
    ...
    def __validate_keys(self, event):
        validate_type = (
            event["type"] in EventType[event["type"]].value
            and event["type"] not in EventType.other.value
        )

        check_str_name = " ".join(
            [i for i in event["name"].split() if len(i) < 20 and i.isalnum()]
        )
