...
input_argument = parser.add_argument(
    "input_json", help="Enter the name of the input json file"
)
output_argument = parser.add_argument(
    "output_json", help="Enter the name of the output json file"
)
...
class RunInConsole:
    get_fired_events = EventsGeneration(
        "2022-09-01", "2023-04-10"
    )  # Объект сгенерированного JSON файла
    ...
    def output_file(self, o_positional, transformation):
        with open(f"./output_json/{o_positional}", "w", encoding="utf8") as output_file:
            json.dump(
                self.get_grouped_events.working_with_events(transformation),
                output_file,
                indent=4,
            )
